function [pPrime, uPrime, ePrime, rhoPrime, APlus, AMinus, EPlus] = Fcn_calculate_u_p_e_rho_perturbations_at_a_give_position(x)
...
pPrime      = APlus + AMinus; % Pressure fluct (Pa)
uPrime      = (APlus - AMinus)./(CI.TP.c_mean(1,indexSection).*CI.TP.rho_mean(1,indexSection)); % Velocity fluct (m/s)
